...
def helper(dis):
  ...
  pr=precaution[precaution['Disease']==dis][['Precaution_1','Precaution_2','Precaution_3','Precaution_4']]
...
# symDICT = {feature: idx for idx, feature in enumerate(selected_features)}
...
def get_predicted_value(patient_symptoms):
  ...
  predicted_dis = mnb.predict([input_value])[0]
  # print(predicted_dis)
  key = next((k for k, v in diseases_list.items() if v == predicted_dis), None)
...
def predict():
    if request.method == "POST":
        ...
        selected_symptoms = request.form.getlist('symptoms')  # Correct way to get multiple selected options

        if not selected_symptoms:
            return render_template('index.html', mymsg="Please select at least one symptom.")

        symptoms = ','.join(selected_symptoms)  # Convert list to comma-separated string
        ...
        return render_template('index.html', predicted_disease=predicted_disease,disDesc=desc,disPrec=my_prec,disMed=my_med,disDiet=my_diet,disWork=work)
